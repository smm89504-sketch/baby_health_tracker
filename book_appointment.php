...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['doctor_id'], $_POST['appointment_time'], $_POST['date'], $_POST['child_id'])) {
    ...
    $stmt = $conn->prepare("SELECT COUNT(*) FROM appointments WHERE doctor_id=? AND DATE(appointment_date)=? AND TIME(appointment_date)=? AND appointment_status IN ('scheduled','confirmed')");
    ...
    if ($count > 0) {
        ...
        // Add appointment
        $stmt = $conn->prepare("INSERT INTO appointments (child_id, doctor_id, parent_id, appointment_date, appointment_type, appointment_status) VALUES (?, ?, ?, ?, 'check-up', 'scheduled')");
        $stmt->bind_param('iiis', $child_id, $doctor_id, $parent_id, $appointment_datetime);
        if ($stmt->execute()) {
